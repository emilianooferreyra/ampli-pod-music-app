...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
  const [mix20Data] = await History.aggregate([
  ...
  if (mix20Data) {
    await Playlist.updateOne(
      ...
      { $set: { title, items: mix20Data.items, visibility: "auto" } },
      { upsert: true }
    );
  }
...
export const getFollowersProfile: RequestHandler = async (req, res) => {
  ...
  res.json({ followers: followersData.followers });
};
...
export const getFollowerProfilePublic: RequestHandler = async (req, res) => {
  ...
  const [profileFollowersData] = await User.aggregate([
  ...
  res.json({ followers: profileFollowersData.followers });
};

export const getFollowingsProfile: RequestHandler = async (req, res) => {
  const { limit = "20", pageNumber = "0" } = req.query as paginationQuery;
  const [followingsData] = await User.aggregate([
    { $match: { _id: req.user.id } },
    {
      $project: {
        followings: {
  ...
  res.json({ followings: followingsData.followings });
};
...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  const [playlistData] = await Playlist.aggregate(aggregationLogic);
  if (!playlistData) {
    const [autoPlaylistData] = await AutoGeneratedPlaylist.aggregate(
      aggregationLogic
    );

    return res.json({ list: autoPlaylistData });
  }
